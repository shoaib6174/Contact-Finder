"""Adapter: Yelp business listing search."""
...
async def yelp_search(name: str, address: str) -> list[dict]:
    """Search Yelp for a business by name and location."""
    if not _config.serper_api_key:
        return [{"error": "SERPER_API_KEY not configured"}]

    query = f'"{name}" site:yelp.com {address}'
    ...
    for item in data.get("organic", []):
        ...
        candidates.append(
            ContactCandidate(
                ...
                source_url=link,
                source_trust=0.75,
                raw_evidence=f"Yelp listing: {item.get('title')}",
            ).model_dump()
        )
